...
const loadCategoryTags = async () => {
  try {
    const {data: categoriesData} = await genericFetch('GET', 'https://localhost:7205/api/Post/GetCategories');
    const {data: tagsData} = await genericFetch('GET', 'https://localhost:7205/api/Post/GetTags');
